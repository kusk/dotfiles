...
open(my $l, "ls -hAlv --time-style='+%d/%m %H:%M' --group-directories-first --color=always @ARGV|")
  or die($!);
...
for(@ls) {
  ...
  for(@perms) {
    ...
    $_ = "$fg;140m$_$def" if($_ eq 'r');
    $_ = "$fg;080m$_$def" if($_ eq 'w');
    $_ = "$fg;035m$_$def" if($_ eq 'x');
  ...
  printf(" $fg;245m%s $fg;238m$right_delimiter$fg;245m%s %s $fg;238m$right_delimiter$fg;139m%s $fg;241m %s\n",
   $perm, $day, $month, $size, $time, $file);
}
exit(0);
